void draw_channels()
{
    ...
    auto file = new TFile(Form("data_reco/pre_li7_%04d.reco.root",LKParameterContainer("run_reco.mac").GetParInt("LKRun/RunID")));
    ...
    for (auto cobo : {1}) {
        for (auto asad : {0}) {
            ...
            for (auto aget : {0,1,2,3})
            {
                ...
                new TH2D(hist_name, cut2+";channels;energy;", 80,0,80,2000,0,5000);
